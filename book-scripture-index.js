...
// ── Live indexes generated by the print builds ─────────────────────────────
// Each JSON maps "Book C:V(-V)" -> [{p: page, q: quoted, ch: printed chapter}]
// plus a "__chapters" map (printed chapter number -> web chapter slug). These
// are the authority; the static map above remains only as a fallback while
...
async function loadBookScriptureIndexes() {
  if (_liveBookIndexes) return _liveBookIndexes;
  const loaded = [];
  for (const src of BOOK_INDEX_SOURCES) {
    try {
      const r = await fetch(src.url);
      if (!r.ok) continue;
      const data = await r.json();
      const chapters = data.__chapters || {};
      delete data.__chapters;
      loaded.push({ bookSlug: src.bookSlug, tag: src.tag, chapters, refs: data });
    } catch (e) { /* offline / missing — the static fallback stays in force */ }
  }
  if (loaded.length) _liveBookIndexes = loaded;
  return _liveBookIndexes;
}

if (typeof window !== 'undefined') {
  // Warm the cache so verse taps have both books' data by first use
  setTimeout(() => { loadBookScriptureIndexes(); }, 0);
}

// Look up book references for a verse.
// New shape (live indexes): {bookSlug, tag, chapterSlug, title, anchor, quoted}
// Legacy shape (static map): {chapter, title, anchor}
function getBookReferences(book, chapter, verse) {
  if (_liveBookIndexes) {
    const out = [];
    for (const idx of _liveBookIndexes) {
      for (const [ref, locs] of Object.entries(idx.refs)) {
        const m = ref.match(/^(.+?)\s+(\d+):(\d+)(?:-(\d+))?$/);
        if (!m) continue;
        const [, refBook, refChapter, startVerse, endVerse] = m;
        ...
        const e = endVerse ? parseInt(endVerse) : s;
        if (v < s || v > e) continue;
        const anchor = `ref-${refBook.trim().toLowerCase().replace(/\s+/g, '')}-${refChapter}-${startVerse}`;
        for (const loc of locs) {
          const slug = idx.chapters[String(loc.ch)];
          if (!slug) continue;
          const helper = (typeof window !== 'undefined' && window.BOOKS_BY_SLUG) ? window.BOOKS_BY_SLUG[idx.bookSlug] : null;
          const chInfo = helper ? helper.getChapter(slug) : null;
          out.push({
            bookSlug: idx.bookSlug,
            tag: idx.tag,
            chapterSlug: slug,
            title: chInfo ? chInfo.title : slug,
            anchor,
            quoted: !!loc.q
          });
        }
    ...
    if (!out.length) return null;   // the live indexes are the authority
    out.sort((a, b) => (b.quoted ? 1 : 0) - (a.quoted ? 1 : 0));
    return out;
  }
