---
layout: symbol-study
record_version: 1
record_type: symbol
symbol_key: four
term: Four winds
title: Four winds — Symbol Research
description: >-
  A Scripture-first study of Four winds: The four spirits of heaven — God's executing agents, shown
  as the four horsemen. The same fourfold outgoing pattern appears in the river dividing from Eden.
words:
  - four winds
definition: >-
  The four spirits of heaven — God's executing agents, shown as the four horsemen. The same fourfold
  outgoing pattern appears in the river dividing from Eden.
meaning: >-
  The four spirits of heaven — God's executing agents, shown as the four horsemen. The same fourfold
  outgoing pattern appears in the river dividing from Eden.
definition_status: approved
definition_meta:
  authority: book-glossary
  status: approved
  source: books/symbolic-language/49-glossary.adoc#sym-four
  verdict: standard
classification: symbol
relationships:
  opposites: []
  related: []
research:
  status: documented
  source_files: []
  corpus:
    translation: KJV with Strong's numbers
    english_forms:
      - four winds
senses:
  - id: approved-core
    status: approved
    summary: >-
      The four spirits of heaven — God's executing agents, shown as the four horsemen. The same
      fourfold outgoing pattern appears in the river dividing from Eden.
    citations:
      - Gen 2:10
      - Zech 6:5
      - Rev 6:1-8
      - Rev 22:1
provenance:
  reconciled_on: "2026-07-13"
  definition_checked_against: books/symbolic-language/49-glossary.adoc
definitions:
  bible_symbolic:
    text: >-
      The four spirits of heaven — God's executing agents, shown as the four horsemen. The same
      fourfold outgoing pattern appears in the river dividing from Eden.
    status: approved
    authority: book-glossary
    source: books/symbolic-language/49-glossary.adoc#sym-four
    citations:
      - Gen 2:10
      - Zech 6:5
      - Rev 6:1-8
      - Rev 22:1
  bible_literal:
    text: null
    status: needs-research
    citations: []
    note: State this from ordinary biblical uses; do not copy the dictionary definition into this field.
  webster:
    text: One more than three; twice two.
    status: quoted
    headword: FOUR
    source: Webster's Revised Unabridged Dictionary (1913)
---

# Four winds

## Summary

**The four spirits of heaven — God's executing agents, shown as the four horsemen. The same fourfold outgoing pattern appears in the river dividing from Eden.**

The fuller book and notebook trail is available in [The Four Winds](/books/symbolic-language/the-four-winds/).

## Definitions

### Symbolic Definition

**The four spirits of heaven — God's executing agents, shown as the four horsemen. The same fourfold outgoing pattern appears in the river dividing from Eden.**

### Literal Biblical Definition

*Not yet stated separately. This is a research gap: the literal sense must be derived from ordinary biblical uses, not copied from Webster.*

### Webster's Definition

Webster's 1913 entry for **four** begins: “One more than three; twice two.”

These definitions may agree, but they are independent evidence. A biblical passage may use the term literally, symbolically, or both at the same time; the literal properties remain part of the logic when Scripture uses the object as a symbol.

## The Short Case

The four spirits of heaven—God's executing agents, shown as the four horsemen.

> *“And a river went out of Eden to water the garden; and from thence it was parted, and became into four heads.”* — Genesis 2:10

> *“And the angel answered and said unto me, These are the four spirits of the heavens, which go forth from standing before the Lord of all the earth.”* — Zechariah 6:5

> *“And I saw when the Lamb opened one of the seals, and I heard, as it were the noise of thunder, one of the four beasts saying, Come and see.”* — Revelation 6:1

> *“And I saw, and behold a white horse: and he that sat on him had a bow; and a crown was given unto him: and he went forth conquering, and to conquer.”* — Revelation 6:2

> *“And when he had opened the second seal, I heard the second beast say, Come and see.”* — Revelation 6:3

> *“And there went out another horse that was red: and power was given to him that sat thereon to take peace from the earth, and that they should kill one another: and there was given unto him a great sword.”* — Revelation 6:4

> *“And when he had opened the third seal, I heard the third beast say, Come and see. And I beheld, and lo a black horse; and he that sat on him had a pair of balances in his hand.”* — Revelation 6:5

> *“And I heard a voice in the midst of the four beasts say, A measure of wheat for a penny, and three measures of barley for a penny; and see thou hurt not the oil and the wine.”* — Revelation 6:6

The definition stays with the relationship these passages establish. Literal appearances preserve the image; they do not require every occurrence to be symbolic.

## Dictionary Comparison

Webster's 1913 entry for **four** begins:

> “Four, a. Etym: [OE. four, fower, feower, AS. feówer; akin to OS. fiwar, D. & G. vier, OHG. fior, Icel. fj, Sw. fyra, Dan. fire, Goth. fidw, Russ. chetuire, chetvero, W. pedwar, L. quatuor, Gr. catur. Farthing, Firkin, Forty, Cater four, Quater-cousin, Quatuor, Quire of paper, tetrarch.] Defn: One more than three; twice two.”

The dictionary supplies the ordinary sense. The definition above is narrower or figurative only where Scripture's own cross-references require that transfer.

## Corpus and Method

This study checks the glossary's defining citations first, then compares them with every KJV verse that prints *four winds*. The English-form sweep found **9 verses**. It is exhaustive for those printed forms, but it is not a substitute for a Strong's-number sweep where several Hebrew or Greek words share one English translation.

## Evidence by Sense

The entry currently states one controlling sense:

1. **Core sense:** The four spirits of heaven — God's executing agents, shown as the four horsemen. The same fourfold outgoing pattern appears in the river dividing from Eden.
2. **Defining witnesses:** Gen 2:10; Zech 6:5; Rev 6:1-8; 22:1
3. **Boundary:** the sense applies where the passage activates the same relationship; ordinary literal uses remain literal.

## Competing Definitions Tested

- Zechariah 6:5 explicitly calls the figures “four spirits of the heavens” and describes them as going forth from before God, supporting the agent identification. However, the printed verse alone does not establish the added identification with “the four horsemen”; that depends on the surrounding chariot-and-horse vision and further interpretation.

- One could argue DIVERGENT: the consensus's central identification is a spatial merism (directions/totality), and the book replaces this with personal agents identified as the four horsemen, an identification absent from all responses; treating an impersonal directional idiom as four specific personal beings could be seen as replacing the referent rather than refining it.

- While Zechariah 6:5 supports the 'four spirits/winds' as executing agents, applying this definition universally across all biblical mentions of the 'four winds' ignores dozens of passages (like Matthew 24:31 or Jeremiah 49:36) where the phrase clearly functions as a geographical idiom for the four compass directions.



These tests identify the boundary the definition must explain. A shared phrase or nearby theme is not enough by itself; the cited passages must establish the proposed identity and account for the countertexts without changing senses whenever the reading becomes difficult.

## Relationship to Other Symbols

No reciprocal opposite is assigned. Related images may share a scene or a consequence without becoming aliases.

This definition does not depend on another glossary term.

## Occurrence Register

<details markdown="1">
<summary>Show the complete 9-verse English-form register</summary>

**Jeremiah:** 49:36

**Ezekiel:** 37:9

**Daniel:** 7:2, 8:8, 11:4

**Zechariah:** 2:6

**Matthew:** 24:31

**Mark:** 13:27

**Revelation:** 7:1

</details>

## Conclusion

**The four spirits of heaven — God's executing agents, shown as the four horsemen. The same fourfold outgoing pattern appears in the river dividing from Eden.**

This is the glossary conclusion. The occurrence register and competing-reading section show where further evidence would strengthen, narrow, or test it without silently changing the definition.
