---
layout: symbol-study
record_version: 1
record_type: word
symbol_key: belief
term: Belief, to believe
title: Belief, to believe — Symbol Research
description: >-
  A Scripture-first study of Belief, to believe: Receiving God's testimony as true and governing,
  then acting upon it. Mere assent can acknowledge a fact; persistent refusal to obey exposes
  unbelief.
words:
  - belief
  - to believe
definition: >-
  Receiving God's testimony as true and governing, then acting upon it. Mere assent can acknowledge
  a fact; persistent refusal to obey exposes unbelief.
meaning: >-
  Receiving God's testimony as true and governing, then acting upon it. Mere assent can acknowledge
  a fact; persistent refusal to obey exposes unbelief.
definition_status: approved
definition_meta:
  authority: book-glossary
  status: approved
  source: books/symbolic-language/49-glossary.adoc#sym-belief
  verdict: word
classification: word
relationships:
  opposites: []
  related: []
research:
  status: documented
  source_files:
    - books/symbolic-language/research/research-faith.md
  corpus:
    translation: KJV with Strong's numbers
    english_forms:
      - belief
      - to believe
senses:
  - id: approved-core
    status: approved
    summary: >-
      Receiving God's testimony as true and governing, then acting upon it. Mere assent can
      acknowledge a fact; persistent refusal to obey exposes unbelief.
    citations:
      - Mark 1:27
      - John 3:36
      - John 12:42-43
      - Rom 10:16
      - Heb 3:18-19
      - Jas 2:19-22
provenance:
  reconciled_on: "2026-07-13"
  definition_checked_against: books/symbolic-language/49-glossary.adoc
definitions:
  bible_symbolic:
    text: No independent symbolic sense is asserted by this glossary entry.
    status: not-applicable
    citations: []
  bible_literal:
    text: >-
      Receiving God's testimony as true and governing, then acting upon it. Mere assent can
      acknowledge a fact; persistent refusal to obey exposes unbelief.
    status: approved
    authority: book-glossary
    source: books/symbolic-language/49-glossary.adoc#sym-belief
    citations:
      - Mark 1:27
      - John 3:36
      - John 12:42-43
      - Rom 10:16
      - Heb 3:18-19
      - Jas 2:19-22
  webster:
    text: >-
      Assent to a proposition or affirmation, or the acceptance of a fact, opinion, or assertion as
      real or true, without immediate personal knowledge; reliance upon word or testimony; partial
      or full assurance without positive knowledge or absolute certainty; persuasion; conviction;
      confidence; as, belief of a witness; the belief of our senses.
    status: quoted
    headword: BELIEF
    source: Webster's Revised Unabridged Dictionary (1913)
---

# Belief, to believe

## Approved Definition

**Receiving God's testimony as [true](/research/symbols/truth/) and governing, then acting upon it. Mere assent can acknowledge a fact; persistent refusal to obey exposes unbelief.**

The fuller book and notebook trail is available in [Love, Knowing, Faith & Belief](/books/symbolic-language/knowing-faith-love-and-belief/), [research-faith.md](/books/symbolic-language/research/research-faith.md).

## Definition Layers

### Bible symbolic sense

*No separate symbolic sense is asserted. This entry recovers a biblical word meaning rather than interpreting an image.*

### Bible literal sense

**Receiving God's testimony as true and governing, then acting upon it. Mere assent can acknowledge a fact; persistent refusal to obey exposes unbelief.**

### Webster's English sense

Webster's 1913 entry for **belief** begins: “Assent to a proposition or affirmation, or the acceptance of a fact, opinion, or assertion as real or true, without immediate personal knowledge; reliance upon word or testimony; partial or full assurance without positive knowledge or absolute certainty; persuasion; conviction; confidence; as, belief of a witness; the belief of our senses.”

These layers may agree, but they are independent evidence. A biblical passage may use the term literally, symbolically, or both at the same time; the literal properties remain part of the logic when Scripture uses the object as a symbol.

## The Short Case

The book defines belief as receiving God’s testimony as governing truth and submitting to it in obedience. Bare factual assent is inadequate, while persistent disobedience reveals unbelief.

> *“And they were all amazed, insomuch that they questioned among themselves, saying, What thing is this? what new doctrine is this? for with authority commandeth he even the unclean spirits, and they do obey him.”* — Mark 1:27

> *“He that believeth on the Son hath everlasting life: and he that believeth not the Son shall not see life; but the wrath of God abideth on him.”* — John 3:36

> *“Nevertheless among the chief rulers also many believed on him; but because of the Pharisees they did not confess him, lest they should be put out of the synagogue:”* — John 12:42

> *“For they loved the praise of men more than the praise of God.”* — John 12:43

> *“But they have not all obeyed the gospel. For Esaias saith, Lord, who hath believed our report?”* — Romans 10:16

> *“And to whom sware he that they should not enter into his rest, but to them that believed not?”* — Hebrews 3:18

> *“So we see that they could not enter in because of unbelief.”* — Hebrews 3:19

> *“Thou believest that there is one God; thou doest well: the devils also believe, and tremble.”* — James 2:19

The definition stays with the relationship these passages establish. Literal appearances preserve the image; they do not require every occurrence to be symbolic.

## Dictionary Comparison

Webster's 1913 entry for **belief** begins:

> “Be*lief”, n. Etym: [OE. bileafe, bileve; cf. AS. geleáfa. See Believe.] 1. Assent to a proposition or affirmation, or the acceptance of a fact, opinion, or assertion as real or true, without immediate personal knowledge; reliance upon word or testimony; partial or full assurance without positive knowledge or absolute certainty; persuasion; conviction; confidence; as, belief of a witness; the belief of our senses. Belief admits of all degrees, from the slightest suspicion to the fullest assurance. Reid.”

The dictionary supplies the ordinary sense. The approved definition above is narrower or figurative only where Scripture's own cross-references require that transfer.

## Corpus and Method

This study checks the glossary's defining citations first, then compares them with every KJV verse that prints *belief*, *to believe*. The English-form sweep found **3 verses**. It is exhaustive for those printed forms, but it is not a substitute for a Strong's-number sweep where several Hebrew or Greek words share one English translation.

## Evidence by Sense

The approved entry currently states one controlling sense:

1. **Approved core:** Receiving God's testimony as [true](/research/symbols/truth/) and governing, then acting upon it. Mere assent can acknowledge a fact; persistent refusal to obey exposes unbelief.
2. **Defining witnesses:** Mark 1:27; John 3:36; 12:42-43; Rom 10:16; Heb 3:18-19; Jas 2:19-22
3. **Boundary:** the sense applies where the passage activates the same relationship; ordinary literal uses remain literal.

## Competing Definitions Tested

A common reading is: **personal trust in Christ, not mere assent — the instrument that receives salvation.**

- The entry may blur the distinction between faith itself and the works or obedience that result from and demonstrate faith. Several cited texts show that genuine belief and obedience are inseparable, but they do not necessarily prove that obedience belongs to the definition of belief rather than being its necessary fruit.

- One could argue the book quietly displaces rather than refines the consensus: mainstream teaching treats obedience as the fruit or evidence of belief, whereas the book makes submission constitutive of belief itself ('saving belief obeys the word believed'), which edges toward the faith-and-works readings that strict 'faith alone' traditions reject — so a DIVERGENT classification is arguable. Additionally, Mark 1:27 (unclean spirits obeying Christ's authority) is an oblique support that does not directly concern human saving belief.

- The book may over-weight obedience/works language at the expense of the consensus primary of fiduciary trust, risking a works-oriented displacement of grace-received faith even while citing the same assent-vs-true-faith texts.



These tests identify the boundary the definition must explain. A shared phrase or nearby theme is not enough by itself; the cited passages must establish the proposed identity and account for the countertexts without changing senses whenever the reading becomes difficult.

## Relationship to Other Symbols

No reciprocal opposite is assigned. Related images may share a scene or a consequence without becoming aliases.

Related definitions used by this entry: [true](/research/symbols/truth/).

## Occurrence Register

<details markdown="1">
<summary>Show the complete 3-verse English-form register</summary>

**Luke:** 24:25

**Philippians:** 1:29

**2 Thessalonians:** 2:13

</details>

## Conclusion

**Receiving God's testimony as [true](/research/symbols/truth/) and governing, then acting upon it. Mere assent can acknowledge a fact; persistent refusal to obey exposes unbelief.**

This is the approved glossary conclusion. The occurrence register and competing-reading section show where further evidence would strengthen, narrow, or test it without silently changing the definition.
