---
layout: symbol-study
record_version: 1
record_type: symbol
symbol_key: bramble
term: Bramble (thorns, thistles, briers)
title: Bramble (thorns, thistles, briers) — Symbol Research
description: >-
  A Scripture-first study of Bramble (thorns, thistles, briers): The curse-ground's fruitless growth
  — worldly care and deceitful riches that choke the word, offer false protection, and end in fire.
words:
  - bramble
definition: >-
  The curse-ground's fruitless growth — worldly care and deceitful riches that choke the word, offer
  false protection, and end in fire.
meaning: >-
  The curse-ground's fruitless growth — worldly care and deceitful riches that choke the word, offer
  false protection, and end in fire.
definition_status: approved
definition_meta:
  authority: book-glossary
  status: approved
  source: books/symbolic-language/49-glossary.adoc#sym-bramble
  verdict: standard
classification: symbol
relationships:
  opposites: []
  related: []
research:
  status: documented
  source_files:
    - books/symbolic-language/research/research-bramble-thorns.md
  corpus:
    translation: KJV with Strong's numbers
    english_forms:
      - bramble
senses:
  - id: approved-core
    status: approved
    summary: >-
      The curse-ground's fruitless growth — worldly care and deceitful riches that choke the word,
      offer false protection, and end in fire.
    citations:
      - Gen 3:18
      - Matt 13:22
      - Judg 9:15
      - 2 Sam 23:6-7
      - Heb 6:8
provenance:
  reconciled_on: "2026-07-13"
  definition_checked_against: books/symbolic-language/49-glossary.adoc
definitions:
  bible_symbolic:
    text: >-
      The curse-ground's fruitless growth — worldly care and deceitful riches that choke the word,
      offer false protection, and end in fire.
    status: approved
    authority: book-glossary
    source: books/symbolic-language/49-glossary.adoc#sym-bramble
    citations:
      - Gen 3:18
      - Matt 13:22
      - Judg 9:15
      - 2 Sam 23:6-7
      - Heb 6:8
  bible_literal:
    text: null
    status: needs-research
    citations: []
    note: State this from ordinary biblical uses; do not copy the dictionary definition into this field.
  webster:
    text: "(Bot.) Defn: Any plant of the genus Rubus, including the raspberry and blackberry."
    status: quoted
    headword: BRAMBLE
    source: Webster's Revised Unabridged Dictionary (1913)
---

# Bramble (thorns, thistles, briers)

## Summary

**The curse-ground's fruitless growth — worldly care and deceitful riches that choke the word, offer false [protection](/research/symbols/shadow/), and end in fire.**

The fuller book and notebook trail is available in [The Parables of the Kingdom](/books/symbolic-language/the-parables-of-the-kingdom/), [research-bramble-thorns.md](https://github.com/timetestedbible/timetestedbible.github.io/blob/main/books/symbolic-language/research/research-bramble-thorns.md).

## Definitions

### Symbolic Definition

**The curse-ground's fruitless growth — worldly care and deceitful riches that choke the word, offer false [protection](/research/symbols/shadow/), and end in fire.**

### Literal Biblical Definition

*Not yet stated separately. This is a research gap: the literal sense must be derived from ordinary biblical uses, not copied from Webster.*

### Webster's Definition

Webster's 1913 entry for **bramble** begins: “(Bot.) Defn: Any plant of the genus Rubus, including the raspberry and blackberry.”

These definitions may agree, but they are independent evidence. A biblical passage may use the term literally, symbolically, or both at the same time; the literal properties remain part of the logic when Scripture uses the object as a symbol.

## The Short Case

The curse-ground’s crop—those consumed by the care of this world and the deceitfulness of riches: fruitless, choking the word, offering a false shadow, and ending in fire.

> *“Thorns also and thistles shall it bring forth to thee; and thou shalt eat the herb of the field;”* — Genesis 3:18

> *“He also that received seed among the thorns is he that heareth the word; and the care of this world, and the deceitfulness of riches, choke the word, and he becometh unfruitful.”* — Matthew 13:22

> *“And the bramble said unto the trees, If in truth ye anoint me king over you, then come and put your trust in my shadow: and if not, let fire come out of the bramble, and devour the cedars of Lebanon.”* — Judges 9:15

> *“But the sons of Belial shall be all of them as thorns thrust away, because they cannot be taken with hands:”* — 2 Samuel 23:6

> *“But the man that shall touch them must be fenced with iron and the staff of a spear; and they shall be utterly burned with fire in the same place.”* — 2 Samuel 23:7

> *“But that which beareth thorns and briers is rejected, and is nigh unto cursing; whose end is to be burned.”* — Hebrews 6:8

The definition stays with the relationship these passages establish. Literal appearances preserve the image; they do not require every occurrence to be symbolic.

## Dictionary Comparison

Webster's 1913 entry for **bramble** begins:

> “Bram”ble, n. Etym: [OE. brembil, AS.brbramal), fr. the same root as E. broom, As. br. See Broom.] 1. (Bot.) Defn: Any plant of the genus Rubus, including the raspberry and blackberry. Hence: Any rough, prickly shrub. The thorny brambles, and embracing bushes. Shak.”

The dictionary supplies the ordinary sense. The definition above is narrower or figurative only where Scripture's own cross-references require that transfer.

## Corpus and Method

This study checks the glossary's defining citations first, then compares them with every KJV verse that prints *bramble*. The English-form sweep found **4 verses**. It is exhaustive for those printed forms, but it is not a substitute for a Strong's-number sweep where several Hebrew or Greek words share one English translation.

## Evidence by Sense

The entry currently states one controlling sense:

1. **Core sense:** The curse-ground's fruitless growth — worldly care and deceitful riches that choke the word, offer false [protection](/research/symbols/shadow/), and end in fire.
2. **Defining witnesses:** Gen 3:18; Matt 13:22; Judg 9:15; 2 Sam 23:6-7; Heb 6:8
3. **Boundary:** the sense applies where the passage activates the same relationship; ordinary literal uses remain literal.

## Competing Definitions Tested

- A MATCH classification is plausible because every component of the book’s interpretation appears explicitly in the consensus primary meaning or listed alternatives. Nevertheless, the book presents these components as a unified and comparatively narrow identification, excluding several other applications admitted by the consensus, so REFINED better captures its extension.

- In Matthew 13:22 the thorns are explicitly identified with 'the care of this world, and the deceitfulness of riches'—the cares themselves—while the unfruitful person is the one who receives seed among thorns; the book reclassifies the thorns as the persons consumed by those cares, which slightly strains that proof-text even though Judges 9:15 and 2 Samuel 23:6-7 do support a personal referent. One could also argue the book's exclusion of the crown of thorns and Paul's thorn omits central consensus material, pushing toward a stronger disagreement than REFINED captures.

- By combining Matthew 13's 'cares of this world' with Judges 9's 'false shadow' and Hebrews 6's 'fire,' the book treats distinct parables as parts of a single, rigid allegorical system, potentially over-harmonizing texts that merely share a common agricultural backdrop.



These tests identify the boundary the definition must explain. A shared phrase or nearby theme is not enough by itself; the cited passages must establish the proposed identity and account for the countertexts without changing senses whenever the reading becomes difficult.

## Relationship to Other Symbols

No reciprocal opposite is assigned. Related images may share a scene or a consequence without becoming aliases.

Related definitions used by this entry: [protection](/research/symbols/shadow/).

## Occurrence Register

<details markdown="1">
<summary>Show the complete 4-verse English-form register</summary>

**Judges:** 9:14, 9:15

**Isaiah:** 34:13

**Luke:** 6:44

</details>

## Conclusion

**The curse-ground's fruitless growth — worldly care and deceitful riches that choke the word, offer false [protection](/research/symbols/shadow/), and end in fire.**

This is the glossary conclusion. The occurrence register and competing-reading section show where further evidence would strengthen, narrow, or test it without silently changing the definition.
