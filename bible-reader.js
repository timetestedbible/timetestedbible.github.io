...
function showVerseTooltip(el, event) {
  ...
  // Use the global normalizeBookName which handles all variations
  // (Psalm→Psalms, Gen→Genesis, etc.)
  const bookAbbr = match[1].trim();
  const book = (typeof normalizeBookName === 'function') ? normalizeBookName(bookAbbr) : bookAbbr;
  const chapter = parseInt(match[2]);
  const verse = parseInt(match[3]);
