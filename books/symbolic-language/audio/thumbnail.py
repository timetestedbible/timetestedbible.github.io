...
def build_bed(src, kind, stem):
    ...
    cover-cropped, never stretched. A BED_TRANSFORMS entry (mirror +
    slide-right) then recomposes the bed so its focal subject sits in the
    right half, clear of the title band."""
...
def compose(stem):
    ...
    img = Image.alpha_composite(build_bed(src, kind, stem), band_overlay())
